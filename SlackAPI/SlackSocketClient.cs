    ...
    public class SlackSocket : ISlackSocket
    {
        ...
        static SlackSocket()
        {
            foreach (var assy in AppDomain.CurrentDomain.GetAssemblies())
            {
                ...
                {
                    bool isHandler = t.GetInterfaces().Any(i => i.IsGenericType && i.GetGenericTypeDefinition() == typeof(IEventHandler<>));
                    if (!isHandler)
                        continue;
        ...
        public void Send<T>(SlackSocketMessage message, Action<T> callback) where T : SlackSocketMessage
        {
